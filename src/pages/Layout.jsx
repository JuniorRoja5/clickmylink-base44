  ...
  const checkUserAndCreator = async () => {
    try {
      ...
      if (creators.length > 0) {
        ...
        const sortedProducts = activeProducts.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-800 hover:bg-black/5 transition-all duration-150 group">
                    ...
                    <div className="flex-1 text-left">
                      <div className="text-sm font-medium text-gray-900 truncate">{creator?.display_name || user?.full_name || 'Usuario'}</div>
                      <div className="text-xs text-gray-500 truncate">@{creator?.username || 'username'}</div>
                    </div>
                ...
                <DropdownMenuContent align="end" side="top" className="w-56 bg-white border-gray-200 shadow-xl rounded-xl p-2" sideOffset={8}>
                  <div className="px-3 py-2 border-b border-gray-100 mb-2">
                    <div className="text-sm font-medium text-gray-900 truncate">{creator?.display_name || user?.full_name}</div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="h-12 glass-card border-b border-gray-200/50 flex items-center justify-between px-6 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              {publicUrl && (<div className="relative flex items-center gap-2 bg-white/50 rounded-full px-3 py-1.5 border border-gray-200 shadow-sm"><a href={publicUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700 font-medium text-sm truncate max-w-[180px]">{publicUrl.replace('https://','').replace('http://','')}</a><Button onClick={handleCopyUrl} size="sm" variant="ghost" className="text-blue-600 hover:text-blue-700 p-1 h-auto rounded-full">{copied ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}</Button><AnimatePresence>{copied && (<motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="absolute right-1/2 translate-x-1/2 -top-8 text-xs font-semibold px-2 py-1 bg-gray-800 text-white rounded-md shadow-lg whitespace-nowrap">Link copiado</motion.div>)}</AnimatePresence></div>)}
            </div>
          </div>
              ...
              <SheetContent side="bottom" className="bg-[#E4F0FE]/95 backdrop-blur-lg border-t border-gray-200/50 rounded-t-2xl">
                <div className="space-y-4 py-6">
                  ...
                  <div className="border-t border-gray-900/10 pt-4 mt-6">
                    <Link to={createPageUrl("Profile")} className="flex items-center gap-4 px-4 py-3 rounded-xl bg-white/40 text-gray-800 hover:bg-white/60 transition-all duration-200">{creator?.avatar_url ? (<img src={creator.avatar_url} alt={creator.display_name} className="w-5 h-5 rounded-full object-cover" />) : (<User className="w-5 h-5" />)}<span className="font-medium">Mi Perfil</span></Link>
