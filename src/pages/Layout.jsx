  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      ...
      <div className="hidden lg:flex h-screen">
        {/* Left Sidebar - Desktop - SLIGHTLY ENLARGED */}
        <div className="w-64 sidebar-glass shadow-2xl flex flex-col">
          {/* Logo Section */}
          <div className="px-4 pt-4 pb-3 border-b border-gray-200">
            <div className="flex items-center gap-2">
              <img
                src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/98cc1bfb6_Screenshot_20250703_182921_Chrome.jpg"
                alt="ClickMyLink"
                className="h-10 w-auto object-contain desktop-logo"
              />
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-between overflow-y-auto">
            {/* Navigation Menu - SLIGHTLY ENLARGED */}
              ...
              <div className="space-y-1.5">
                {mainNavigationItems.map((item) => (
                  <Link
                    key={item.title}
                    to={item.url}
                    className={`sidebar-nav-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
              ...
              <DropdownMenu>
                  ...
                  <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-800 hover:bg-black/5 transition-all duration-150 group">
                    <div className="relative">
                      {creator?.avatar_url ? (
                        ...
                        <div className="w-9 h-9 bg-gray-300 rounded-full flex items-center justify-center border-2 border-transparent">
                    ...
                    <div className="flex-1 text-left">
                      <div className="text-sm font-medium text-gray-900 truncate">
                        {creator?.display_name || user?.full_name || 'Usuario'}
                      </div>
                ...
                >
                  <div className="px-3 py-2 border-b border-gray-100 mb-2">
                    <div className="text-sm font-medium text-gray-900 truncate">
                      {creator?.display_name || user?.full_name}
                    </div>
                    <div className="text-xs text-gray-500 truncate">
                      {user?.email}
                    </div>
                  ...
                  <DropdownMenuItem className="flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg cursor-pointer">
                  ...
                  <DropdownMenuItem
                    ...
                    className="flex items-center gap-3 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg cursor-pointer"
                  >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="h-12 glass-card border-b border-gray-200/50 flex items-center justify-between px-6 shadow-sm">
            <div>
              <h1 className="text-lg font-semibold text-gray-900">
                {currentPageName === 'Dashboard' && 'Home'}
                {currentPageName === 'orders' && 'Ingresos'}
                {currentPageName === 'LinkPreview' && 'Mi Tienda'}
                {currentPageName === 'Design' && 'Diseño'}
                {currentPageName === 'Customers' && 'Clientes'}
                {currentPageName === 'Statistics' && 'Estadísticas'}
                {currentPageName === 'Referrals' && 'Referidos'}
                {currentPageName === 'Calendar' && 'Calendario'}
                {currentPageName === 'EmailSequences' && 'Secuencias de Email'}
                {currentPageName === 'InstagramAutomation' && 'Auto-respuestas IG'}
                {currentPageName === 'AskLink' && 'Pregúntale a Link'}
                {currentPageName === 'Profile' && 'Perfil'}
              </h1>
            </div>
              ...
              {publicUrl && (
                <div className="relative flex items-center gap-2 bg-white/50 rounded-full px-3 py-1.5 border border-gray-200 shadow-sm">
                    <a
                      ...
                      className="text-blue-600 hover:text-blue-700 font-medium text-sm truncate max-w-[180px]"
                    >
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="ghost"
                    className="text-blue-600 hover:text-blue-700 p-1 h-auto rounded-full"
                  >
                  ...
                  {copied && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, x: -10 }}
                        className="absolute right-1/2 translate-x-1/2 -top-8 text-xs font-semibold px-2 py-1 bg-gray-800 text-white rounded-md shadow-lg whitespace-nowrap"
                      >
                        Link copiado
                      </motion.div>
                    )}
          ...
          {/* Page Content */}
          <main className="flex-1 overflow-auto bg-gradient-to-br from-slate-50/50 via-white/30 to-blue-50/20">
            {children}
          </main>
        </div>
      </div>
      ...
      <div className="lg:hidden">
        ...
        <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-200/50 p-3 flex items-center justify-between">
          ...
          {/* Right Side: Action buttons in correct order */}
          <div className="flex items-center gap-1">
            {publicUrl && (
              <div className="relative">
                <Button
                  ...
                  size="icon"
                  variant="ghost"
                  className="text-gray-600 hover:bg-gray-100 h-8 w-8 rounded-full"
                >
                ...
                <AnimatePresence>
                  {copied && (
                      <motion.div
                        ...
                        className="absolute left-1/2 -bottom-10 text-xs font-semibold px-2 py-1 bg-gray-800 text-white rounded-md shadow-lg whitespace-nowrap"
                      >
                        Link copiado
                      </motion.div>
                    )}
            ...
            {shouldShowPreviewButton && (
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 h-8 bg-white border border-gray-300 rounded-md text-gray-600 hover:bg-gray-50 transition-colors"
              >
                ...
                <div className="space-y-4 py-6">
                  ...
                  {mainNavigationItems.slice(4).map((item) => (
                    <Link
                      key={item.title}
                      to={item.url}
                      className="flex items-center gap-4 px-4 py-3 rounded-xl bg-white/40 text-gray-800 hover:bg-white/60 transition-all duration-200"
                    >
                  ...
                  <div className="border-t border-gray-900/10 pt-4 mt-6">
                    ...
                    <Button
                      variant="outline"
                      onClick={handleLogout}
                      className="w-full mt-3 bg-transparent border-red-500 text-red-500 hover:bg-red-50"
                    >
        ...
        {/* Mobile Preview Dialog - INTERNAL PREVIEW */}
              ...
              <div className="relative w-full h-full bg-white rounded-[2.6rem] overflow-hidden">
                {creator ? (
                  <div
                    ...
                    style={{
                      background: creator.design_settings?.background || '#000000',
                      scrollbarWidth: 'none',
                      msOverflowStyle: 'none'
                    }}
                  >
                    <style>{`
                      .w-full.h-full.overflow-y-auto::-webkit-scrollbar {
                        display: none;
                      }
                    `}</style>
                    {previewMode === 'profile' ? (
...
export default function Layout({ children, currentPageName }) {
  // El wrapper <LanguageProvider> ha sido eliminado.
  return (
